...
class SSNetRegv1(nn.Module):
    def __init__(self, class_num, det_chns=(32, 80)):
        super(SSNetRegv1, self).__init__()
        self.stage0 = nn.Sequential(
            nn.Conv2d(3, 24, kernel_size=3, stride=1, padding=1, bias=False),
            nn.MaxPool2d(kernel_size=3, stride=1, padding=1),
            InvertedResidual(24, 24, 1, 1)
        ...
        # chn_in = 80 + 72 + 40, chn_out = 192 + 192 // 8 = 216
        self.enhance_last = GlobalAvgContextEnhanceBlock(chn_in=192, size_hw_in=(24, 18), chn_shrink_ratio=8)
        # for features from detection part, out channel = 112 // 4 = 28
        self.context_embedding = GlobalAvgContextEmbeddingBlock(chn_in=det_chns, size_hw_in=(24, 18), chn_shrink_ratio=2)

        # network out, 216 + 56 = 272
        fc_out_chn = 272
        post_in_chn = fc_out_chn
        post_out_chn = post_in_chn // 2
        self.postprocessor = nn.Sequential(
    ...
    def forward(self, x, x_det1, x_det2):
        x0 = self.stage0(x)  # up to 0.015
        ...
        # print(x_cat.shape)
        x = self.enhance_last(x_concat)  # up to 0.066, out size(1, 216, 24, 24)
        x_context = self.context_embedding([x_det1, x_det2])
...
class SSNetRegv2(nn.Module):
    def __init__(self, class_num, det_chns=32):
        super(SSNetRegv2, self).__init__()
        self.stage0 = nn.Sequential(
            nn.Conv2d(3, 24, kernel_size=3, stride=1, padding=1, bias=False),
            nn.MaxPool2d(kernel_size=3, stride=1, padding=1),
            InvertedResidual(24, 24, 1, 1)
        ...
        self.conv1x1 = conv_1x1_bn(det_chns, 24)
        # chn_in = 80 + 72 + 40 + 24, chn_out = 216 + 216 // 8 = 243
        self.enhance_last = GlobalAvgContextEnhanceBlock(chn_in=216, size_hw_in=(24, 18), chn_shrink_ratio=8)
    ...
    def forward(self, x, x_det):
        ...
        x_det = self.conv1x1(x_det)
        x_det = F.interpolate(x_det, (24, 18), mode='bilinear', align_corners=True)
        x_concat = torch.cat([self.avg_context1(x1), self.avg_context2(x2), x3, x_det], dim=1)
        # print(x_cat.shape)
        x = self.enhance_last(x_concat)  # up to 0.066, out size(1, 216, 24, 24)
        x = self.container(self.postprocessor(x))  # size(1, class_num, 6, 12)
        logits = torch.mean(x, dim=2)  # size(1, class_num, 12)""""""
