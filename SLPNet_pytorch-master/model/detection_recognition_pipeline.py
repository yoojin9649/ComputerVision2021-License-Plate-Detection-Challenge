...
from model.SSNet_modules import SSNetDet, SSNetRegOriginal, SSNetRegv2, SSNetRegv2
from model.other_models.shufflenet import ShuffleNetV2
import module.det_part.GTProcessing as gtP
from module.det_part.PostProcessing import detection_analysis, nms_gauss, clac_gauss_score_multi
import module.decoder as decoder
from model.basic_modules import *
from module.det_part.perspective_transform import PerspectiveTrans
...
class DetectionRecognitionPipeline(nn.Module):
    def __init__(self, input_size=(1024, 1024), det_size=(512, 512), reg_size=(144, 48), class_num=68):
        ...
        # self.model_det = OtherDetectionNet(model='shufflenet', mode='large')
        # self.model_reg = SSNetReg(class_num=class_num, det_chns=(32, 80))
        # self.model_reg = SSNetRegv2(class_num=class_num, det_chns=32)
        ...
        if mode1 == 'det_only':
            ...
            for batch_idx in range(len(obj_num_list)):  # each image in batch
                ...
                if obj_num != 0:
                    ...
                    """
                    det_corners1 = (single_img_corners.reshape(-1, 4, 2) * torch.tensor([w_ratio_det1,
                                                                                        h_ratio_det1]).to(device)).\
                        reshape(-1, 8)
                    det_size1 = (int(self.det_size[0] * w_ratio_det1), int(self.det_size[1] * h_ratio_det1))
                    det_map_list1.extend(self.perspective_trans_function(fea_maps=x1[batch_idx],
                                                                         corners_tensor=det_corners1,
                                                                         target_size=det_size1))
                    det_corners2 = (single_img_corners.reshape(-1, 4, 2) * torch.tensor([w_ratio_det2,
                                                                                        h_ratio_det2]).to(device)).\
                        reshape(-1, 8)
                    det_size2 = (int(self.det_size[0] * w_ratio_det2), int(self.det_size[1] * h_ratio_det2))
                    det_map_list2.extend(self.perspective_trans_function(fea_maps=x2[batch_idx],
                                                                         corners_tensor=det_corners2,
                                                                         target_size=det_size2))"""
            reg_batch = len(lp_img_tensor_list)
            if reg_batch != 0:  # 有lp图，可以进行识别
                # 制作tensor数据
                # print("???")
                lp_imgs_tensor = torch.cat(lp_img_tensor_list, dim=0)
                # print(lp_imgs_tensor.shape)
                # det_maps_tensor1 = torch.cat(det_map_list1, dim=0)
                # det_maps_tensor2 = torch.cat(det_map_list2, dim=0)
                # t0 = time.time()
                logits = self.model_reg(lp_imgs_tensor)
                # t1 = time.time()
                # print("reg time: %.4f" % (t1 - t0))
                # logits = self.model_reg(lp_imgs_tensor, det_maps_tensor1)
                # logits = self.model_reg(lp_imgs_tensor, det_maps_tensor1, det_maps_tensor2)
                if mode2 == 'train':
