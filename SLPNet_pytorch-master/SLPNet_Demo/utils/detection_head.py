...
from utils import GTProcessing as gtP
...
def detection_target(output_maps, corners_list_512, stage_lvl=4):
    ...
    for img in range(num_imgs):
        ...
        for obj in range(obj_num):
            ...
            i_spatial_map = i_spatial_map - ((sample_area_target[img] != 0).byte() * i_spatial_map)
            # print(i_spatial_map[i_spatial_map == 1].size())
            # print(i_spatial_map.shape)
            ...
            with open('2.txt', 'w') as f:
                for y in range(e_spatial_map.shape[0]):
                    for x in range(e_spatial_map.shape[1]):
                        f.write(str(e_spatial_map[y][x].item()))
                        f.write(' ')
                    f.write('\n')
                f.write('\n=======================\n')
                for y in range(i_spatial_map.shape[0]):
                    for x in range(i_spatial_map.shape[1]):
                        f.write(str(i_spatial_map[y][x].item()))
                        f.write(' ')
                    f.write('\n')
                f.write("\n****************************\n")
                for y in range(sample_area_target[img].shape[0]):
                    for x in range(sample_area_target[img].shape[1]):
                        f.write(str(sample_area_target[img][y][x].item()))
                        f.write(' ')
                    f.write('\n')
                f.write('\n=======================\n')
            """
